#!/usr/bin/env python3
"""
IMPORTADOR DE CATÁLOGO PIEZAUTO - Hito 2 v5.4
Procesa archivos XLSX normalizados del COO y los carga en Supabase

VERSIÓN 5.4: RECONEXIÓN MÁS FRECUENTE para timeout agresivo Supabase
- Reconecta cada 30 batches (~3000 SKUs o ~30 min de conexión)
- Timeout real observado: ~2.5 horas pero inconsistente
- Batches: 100 SKUs + delay 2seg entre batches
- Formato PZ-XXXXXX (6 dígitos) con ORDER BY numérico
...
def ingestar_proveedor(filepath: Path, nombre_proveedor: str) -> EstadisticasIngesta:
    ...
    try:
        ...
        BATCH_SIZE = 100  # v5.2: Reducido a 100 para evitar timeouts Supabase
        ...
        for batch_num in range(total_batches):
            ...
            # v5.4: RECONEXIÓN cada 30 batches para evitar timeout de Supabase (~30 min)
            if batch_num > 0 and batch_num % 30 == 0:
                logger.info(f"[RECONEXIÓN] Batch {batch_num + 1}: cerrando y reabriendo conexión...")
                db.cerrar()
                if not db.conectar():
                    raise Exception("No se pudo reconectar a Supabase")
            ...
            try:
                ...
                # v5.2: Delay de 2 segundos entre batches para evitar saturar Supabase
                time.sleep(2)
            except Exception as e:
                db.rollback()
        ...
        if skus_hijos_pre_armados:
            ...
            for batch_num in range(total_batches_hijos):
                ...
                # v5.4: RECONEXIÓN cada 30 batches
                if batch_num > 0 and batch_num % 30 == 0:
                    logger.info(f"[RECONEXIÓN] Batch hijos {batch_num + 1}: cerrando y reabriendo conexión...")
                    db.cerrar()
                    if not db.conectar():
                        raise Exception("No se pudo reconectar a Supabase")
                ...
                try:
                    ...
                    # v5.2: Delay de 2 segundos entre batches
                    time.sleep(2)
                except Exception as e:
                    db.rollback()
