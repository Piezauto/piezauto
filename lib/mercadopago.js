...
// Estado: ESTRUCTURA LISTA — esperando MP_ACCESS_TOKEN productivo.
//
...
async function mpCrearPreferencia({ operacionId, items, pagador, backUrls }) {
  ...
  const payload = {
    ...
    back_urls: {
      success: backUrls.success || `${window.location.origin}/gracias.html?op=${operacionId}&pago=aprobado`,
      failure: backUrls.failure || `${window.location.origin}/gracias.html?op=${operacionId}&pago=fallido`,
      pending: backUrls.pending || `${window.location.origin}/gracias.html?op=${operacionId}&pago=pendiente`,
    },
  ...
  */
  ...
  return null; // Placeholder hasta que el token esté disponible
}
